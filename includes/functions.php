...
function wpbitly_generate_shortlink($post_id) {
    ...
    $dataset = get_post_meta($post_id, '_wpbitly', true);

    if (is_array($dataset)) {    
        foreach ($dataset as $data) {
            if ($data['server'] === WPBITLY_ENV) {
    ...
    if (is_array($response)) {
        $set = is_array(get_post_meta($post_id, '_wpbitly', true)) ? get_post_meta($post_id, '_wpbitly', true) : [];
        ...
        update_post_meta($post_id, '_wpbitly', $set);
    }
